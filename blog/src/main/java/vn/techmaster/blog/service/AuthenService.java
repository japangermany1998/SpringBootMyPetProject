...
public class AuthenService implements IAuthenService {
    ...
    @Override
    public void addRoleSupporter() {
        Role role = roleRepository.findByName("supporter").get();
        User user = userRepository.findByEmail("admin@techmaster.vn").get();
    ...
    @Override
    public Role getRoleuser(String role) {
        ...
        if (cookies != null) {
            for (var cookie : cookies) {
                if (cookie.getName().equals(LOGIN_COOKIE)) {
                    ...
                    if (optionalUser.isPresent()) {
                        User user = optionalUser.get();
                        if (!user.getRoles().contains(roleRepository.findByName("customer").get()) && user.getRoles().size() == 0) {
